...
class MicrocontrollerDataset(Dataset):
    def __init__(self, dir_path, width, height, classes, transforms=None):
        ...
        self.all_images = [image_path.replace('\\','/').split('/')[-1] for image_path in self.image_paths]
        self.all_images = sorted(self.all_images)
        
    def __getitem__(self, idx):
        # capture the image name and the full image path
        image_name = self.all_images[idx]
        image_path = os.path.join(self.dir_path, image_name).replace("\\","/")
        # read the image
        image = cv2.imread(image_path)
        # convert BGR to RGB color format
        image = cv2.cvtColor(image, cv2.COLOR_BGR2RGB).astype(np.float32)
        # resizes image to desired size and scales the image pixels
        ...
        # this is where we parse the xml file
...
test_img_thing = cv2.imread("../Microcontroller Detection/train/IMG_20181228_101826.jpg")

# execute datasets.py using Python command from Terminal to visualize sample images
